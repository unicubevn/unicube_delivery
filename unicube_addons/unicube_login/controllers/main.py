...
class Home(http.Controller):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        style = param_obj.get_param('unicube_login.style')
        background = param_obj.get_param('unicube_login.background')
        values['background_color'] = param_obj.get_param('unicube_login.color')
        background_image = param_obj.get_param('unicube_login.background_image')
        component_image = param_obj.get_param('unicube_login.component_image')

        if background == 'image':
            bg_image_url = ''
            com_image_url = ''
            if background_image and component_image:
                base_url = param_obj.get_param('web.base.url')
                bg_image_url = base_url + '/web/image?' + 'model=login.image&id=' + background_image + '&field=image'
                com_image_url = base_url + '/web/image?' + 'model=login.image&id=' + component_image + '&field=image'
                values['background_src'] = bg_image_url or ''
                values['background_color'] = ''
                values['background_component'] = com_image_url or ''
        ...
        if style == 'default' or style is False:
            ...
            response = request.render('unicube_login.left_login_template', values)
        elif style == 'right':
            response = request.render('unicube_login.right_login_template', values)
        else:
            response = request.render('unicube_login.middle_login_template', values)
